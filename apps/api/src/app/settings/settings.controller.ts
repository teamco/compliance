...
export class SettingsController {
  ...
  @Patch('me')
  async updatePrefs(@Req() req: AuthedRequest, @Body() body: Partial<UserPrefsPayload>) {
    const uid = req.user?.uid;
    if (!uid) throw new UnauthorizedException();
  ...
  @Post('push')
  async savePushSub(@Req() req: AuthedRequest, @Body() body: PushSubscriptionPayload) {
    const uid = req.user?.uid;
    if (!uid) throw new UnauthorizedException();
  ...
  @Delete('push')
  async removePushSub(@Req() req: AuthedRequest, @Body() body: { endpoint: string }) {
    const uid = req.user?.uid;
    if (!uid) throw new UnauthorizedException();
