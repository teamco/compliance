...
**Framework catalog is confirmed exempt, not just assumed.** `Framework` (`libs/shared/src/strategies/notes.ts:54`) has no `orgId` field anywhere in its schema — it's a platform-wide reference catalog (SOC2, ISO27001, etc.), and `listControlsByFramework` (`supabase-notes.strategy.ts:966`) queries the `controls` catalog table with no org filter, confirming `frameworks/:id/controls` is correctly org-agnostic. `frameworks`, `frameworks/:id`, `frameworks/:id/requirements` stay untouched. This is a narrower exemption category than Phase 1's (which exempted routes with an existing resource-specific identity check); here it's "the resource itself has no org dimension," and it applies only to the 3 pure-catalog routes.
