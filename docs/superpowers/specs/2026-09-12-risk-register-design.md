...
**Risk ID generation:** server-side, per-org sequential counter (`SELECT max(...) + 1` inside the insert transaction, matching how the Controls migration would have needed to but didn't need — Controls used user-supplied codes; Risk IDs must be system-generated and collision-free under concurrent creates, so use a Postgres sequence or `nextval` per org rather than a naive max+1 read).
...
- **Ambiguity check:** "one active acceptance at a time" resolved explicitly above (most recent non-terminal-expired wins). Risk ID generation concurrency resolved (sequence, not read-then-write). Evidence reuse vs new type resolved (extend existing).
