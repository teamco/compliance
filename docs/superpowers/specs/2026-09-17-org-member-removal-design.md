...
This is where enforcement actually happens: `checkOrgAccess` and `checkOrgManage` (`apps/api/src/app/auth/auth.controller.ts:334-369`, and the duplicate copy in `notes.controller.ts`) both resolve the caller's membership by calling `listOrgMembers` and searching for the caller's `userId`. Once `listOrgMembers` excludes deactivated rows, a deactivated member fails both checks on their very next request. This holds for every route gated by `checkOrgAccess`/`checkOrgManage`. It does **not** reach routes that check assignee identity directly instead of org membership — this codebase's own `route-coverage.unit.test.ts` enumerates these as `EXEMPT_ROUTES` (assessment approval, issue-validation review, risk-acceptance decisions, exception-renewal requests, and similar). A removed member holding a live token can still act on those routes if they were the named assignee before removal. Closing that gap is out of scope for this feature — tracked as a follow-up.
...
- Bulk removal.
- Revoking access for a removed member on routes that check assignee identity directly instead of org membership (`EXEMPT_ROUTES` in `route-coverage.unit.test.ts`: assessment approval, issue-validation review, risk-acceptance decisions, exception-renewal requests, and similar). A removed member can still act on those routes if they were the named assignee before removal — tracked as a follow-up.
