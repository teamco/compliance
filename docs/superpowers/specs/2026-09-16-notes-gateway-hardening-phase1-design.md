...
Out of scope (explicit, tracked separately in project memory): Phase 2 covers InternalControl, Framework, Standards, Policy, Gap analysis — a separate spec/plan when this phase ships. Two routes already resolve org correctly via a custom inline org-creator-OR-party check (`listExceptionRenewals`, `listIssueValidations`) and are genuinely left alone — not vulnerable, just stylistically inconsistent with the shared helper, and rewriting working code purely for style is out of scope. A general principle, discovered mid-execution and applied retroactively: a route whose existing check already ties authorization to a DB-stored, non-attacker-controlled identity field on the resource itself (an owner, an approver, an assigned validator) is already complete, resource-specific authorization independent of org membership. Layering `checkOrgAccess` (creator-only for non-admins) on top of such a check adds no real security value today and would wrongly block a legitimate non-creator/non-admin org member from acting on a resource they're already individually authorized for, once real multi-member orgs exist. Three routes fall into this bucket and get NO `checkOrgAccess` added, left exactly as-is: `requestExceptionRenewal` and `submitIssueForValidation` (owner-only check, `if (resource.ownerId !== userId) throw new ForbiddenException()`, enforced in the gateway controller), and `reviewIssueValidation` (validator-only check, `validation.validatorId !== validatorId → issue_validation_not_authorized_validator`, enforced at the strategy layer, not the controller).
